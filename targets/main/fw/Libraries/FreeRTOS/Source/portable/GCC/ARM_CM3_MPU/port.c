...
//This is a fix for bug http://sourceforge.net/tracker/?func=detail&aid=3035224&group_id=111543&atid=659633
//#define portNVIC_SVC_PRI						( ( ( unsigned long ) configKERNEL_INTERRUPT_PRIORITY ) << 24UL )
#define portNVIC_SVC_PRI						( ( ( unsigned long ) (configMAX_SYSCALL_INTERRUPT_PRIORITY - (2 << 4)) ) << 24UL )
 ...
 */
static void prvSVCHandler( unsigned long *pulRegisters ) __attribute((used)) __attribute__(( noinline )) PRIVILEGED_FUNCTION;
...
static void prvSetupMPU( void )
{
	...
	if( *portMPU_TYPE == portEXPECTED_MPU_TYPE_VALUE )
	{
		...
		*portMPU_REGION_ATTRIBUTE =		( portMPU_REGION_PRIVILEGED_READ_ONLY ) |
		...
		// Setup the RAM region. Attention: This is not the default configuration of FreeRTOS!
		...
		*portMPU_REGION_ATTRIBUTE =		( portMPU_REGION_READ_WRITE) |
										( portMPU_REGION_CACHEABLE_BUFFERABLE ) |
										prvGetMPURegionSizeSetting( ( unsigned long ) __SRAM_segment_end__ - ( unsigned long ) __privileged_data_start__ ) |
										( portMPU_REGION_ENABLE );
