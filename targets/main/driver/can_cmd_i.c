...
static uint8_t u_my_can_id = 0xff;
static volatile xQueueHandle xQueueCANRX;

#ifdef HWP_CAN_MASTER
static uint8_t read_slave_memory_slave_id = 0;
static uint8_t read_slave_memory_num_slaves = 0;
static uint32_t read_slave_memory_address = 0;
static uint32_t *p_read_slave_memory_data = 0;
static CAN_CMD_I_RESULT_T *p_read_slave_memory_notifier = NULL;
static CLOCK_MS_T read_slave_memory_timestamp = 0;

static xSemaphoreHandle can_cmd_i_read_memory_master_mutex;
static xSemaphoreHandle can_cmd_i_read_memory_semaphore;

//SLAVE_DATA_LIST_ITEM_T slave_data_list[CAN_CMD_I_HIGHEST_SLAVE_ID_ALLOWED];
static SLAVE_DATA_LIST_ITEM_T *slave_data_list;
#endif
