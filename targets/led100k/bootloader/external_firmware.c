 ...
 * @param  selected_fw: firmware to erase (main or zigbee)
 ...
 */bool EXT_FW_CheckNotEmpty(FWT_SELECTED_FW_T selected_fw) {
	...
	case FWT_SELECTED_FW_MAIN:
		...
		break;
	case FWT_SELECTED_FW_ZIGBEE:
		FlashAddress = FLASH_ZIGBEE_FW_BASE;
		break;
	default:
		return FALSE;
	}
 ...
 */bool EXT_FW_GetImageInformation(FWT_SELECTED_FW_T selected_fw,
	...
	case FWT_SELECTED_FW_ZIGBEE:
		FlashAddress = FLASH_ZIGBEE_FW_BASE;
		FlashEndAddress = FLASH_ZIGBEE_FW_END;
		ImageInformationAddress = ZIGBEE_IMAGE_INFO_STRUCT_OFFSET;
		break;
	default:
		return FALSE;
	}
	...
	while (FlashAddress < FlashEndAddress) {
		// read data from external flash
		if (!SPI_SERIAL_FLASH_ReadBuffer(FlashAddress, (void*) buffer,
				BLOCK_SIZE))
			return FALSE;
		...
		for (uint16_t i_ctr = 0; i_ctr < BLOCK_SIZE; i_ctr++, iByte++) {
			if (iByte == ImageInformationAddress) {
				...
				memcpy(ImageInformation, &buffer[i_ctr],
						sizeof(IMAGE_INFORMATION_T));
				return TRUE;
			}
		}
	}
 ...
 */bool EXT_FW_ExtImageIsValid(FWT_SELECTED_FW_T selected_fw) {
	...
	while ((FlashAddress < FlashEndAddress) && (!b_finished)) {
		...
		for (uint16_t i_ctr = 0; i_ctr < BLOCK_SIZE; i_ctr++, iByte++) {
			...
			if (b_extracted_image_information
					&& (iByte == ImageInformation.size - 1)) {
				b_finished = TRUE;
	...
	if (ImageInformation.size
			> (uint32_t) (_image_flash_end - (uint8_t*) IMAGE_INFO_BASE))
		return FALSE;
