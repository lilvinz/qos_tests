...
volatile RTCTIME	isr_rtc_unixtime;

volatile RTCTIME	_rtc_unixtime;
RTCTM	_rtc_datetime;

bool	_rtc_unixtime_isvalid;
bool	bUpdateDatetime;
...
#if 0
void clock_set_unixtimestamp(uint32_t unixtimestamp)
{
    clock_unixtimestamp = unixtimestamp;
}

uint32_t clock_get_unixtimestamp(void)
{
    return clock_unixtimestamp;
}

void clock_set_unixtime(uint8_t second, uint8_t minute, uint8_t hour,
                        uint8_t day, uint8_t month, uint16_t year,
                        int16_t offset)
{
    volatile int64_t   yo;
    volatile int32_t   jul;
    ...
    // ermittle Tage seit 1.1.1970
    jul = 36525L * year / 100 + (306001L * (month + 1)) / 10000 + day	- 719590L + yo / 4 - yo;
    yo = ((jul * 24 + hour - 19) * 60 + minute) * 60 + second - 18000;
    yo += second + (60 * (minute + 60 * hour));
    clock_unixtimestamp = (uint32_t)yo + offset;
}

void clock_increase_unixtime(void)
{
    clock_unixtimestamp++;
}
